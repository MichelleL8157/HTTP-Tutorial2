...
app.post('/api/courses', (req,res) => {
    ...
    if (name.length <= 3) {
        res.status(404).send("Name of the course must be minimum of 3 characters.");
    } else {
        const course ={
...
app.put('/api/courses/:id', (req,res)=>{
    ...
    if (!course) {
        ...
        if(req.body.id != req.params.id){
            res.send("Request ID not equal to JSON ID");
        } else {
...
app.delete('/api/courses/:id', (req,res)=>{
    ...
    if (!course) {
        ...
        if(req.body.id != req.params.id){
            ...
            var deleteC = courses.find(c => c.id == req.params.id)
            console.log(courses.indexOf(deleteC));
            console.log(courses.splice(courses.indexOf(deleteC),1));
            res.send(courses);
        }
